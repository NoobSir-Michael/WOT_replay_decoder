...
namespace wotreplay {
     ...
     */
    class game_t {
        friend class parser_t; // 使用友元, 使用parser对象直接操纵game内部的数据
    public:
        /**
        ...
        const std::map<int, player_t> &get_players() const;
    private:
        std::vector<packet_t> packets; // 所有包的列表
        std::array<std::set<int>, 2> teams; // 两个set
        std::string game_mode; // "ctf"
        arena_t arena;
        // 下面四个都是用于存放回放元数据的buffer
        buffer_t game_begin;
        buffer_t player_info;
        buffer_t game_end;
        buffer_t replay;

        uint32_t recorder_id; // 5134305 从文件中获取的记录的用户id
        version_t version; // major=16, minor=1 -- 1.16.1
