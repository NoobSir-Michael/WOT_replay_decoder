...
std::ostream& packet_t::packetContentOutput(std::ostream& os) const {
    uint32_t length = get_field<uint32_t>(data.begin(), data.end(), 0);
    uint32_t distance = std::distance(data.begin(), data.end());
    os<<"length = "<<length<<std::endl;
    os<<"distance = "<<distance<<std::endl;
    for (size_t i = 0; i < distance/4; i++)
    {
        uint32_t content_32 = get_field<uint32_t>(data.begin(), data.end(), i*4);
        float countent_float = get_field<float>(data.begin(), data.end(), i*4);
        if (countent_float < 1e-30 && 0 < countent_float)
        {
            countent_float = 0;
        }
        
        os<<"#"<<4*i<<"\t"<<content_32<<"\t"<<countent_float<<std::endl;
    }
    return os;
}

uint32_t packet_t::get_packet_distance() const{
    uint32_t distance = std::distance(data.begin(), data.end());
    return distance;
}