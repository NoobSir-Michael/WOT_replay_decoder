NoobSir-Michael 修改内容
==============
## 基本功能与修改内容
1. 删除了对于图像的输出以及对于地图文件的依赖代码
2. 输出格式为`.csv`文件, 并存放在项目根目录的`output`文件夹下
3. 输出的内容为两个`csv`文件, 一个包含文字聊天信息, 一个包含用户以及游戏中坦克位置信息
4. 删除了输出相关的命令
## 需要后续开发的部分:
1. 代码内部有对于WoWS的密钥, 但是没有对于WoWS的回放解码功能
2. 除了坦克位置和文字聊天, 其他的解码功能不再适用于新版游戏的回放.
3. CMake中的图像输出的依赖项没有删除
## 代码使用
1. 依赖安装: `sudo apt-get install build-essential libboost-all-dev git cmake libpng-dev libcrypt-dev libssl-dev libgd-dev libtbb-dev`
2. 编译: `cmake . && make`
3. 运行: `bin/wotreplay-parser --input <PATH TO .WOTREPLAY FILE>`


原版: World of Tanks Replay Parser
==============================================================
